...
void InterfaceSolver::SwapData()
{
    for ( int i = 0; i < this->neighbor_datas.size(); ++ i )
    {
        ...
        {
            int n = idata.interface_neighbor_faceids.size();
            MPI_Send(&n, 1, MPI_INT, ip, tag, MPI_COMM_WORLD);
            for ( int i = 0; i < idata.interface_neighbor_faceids.size(); ++ i )
            {
                std::printf( "1: idata.interface_neighbor_faceids[%d]=%d \n", i, idata.interface_neighbor_faceids[ i ] );
            }
            MPI_Send(idata.interface_neighbor_faceids.data(), n, MPI_INT, ip, tag, MPI_COMM_WORLD);
        }
        else
        {
            int n = -1;
...
void InterfaceSolver::SwapData( float * q )
{
    ...
    for ( int i = 0; i < this->neighbor_datas.size(); ++ i )
    {
        ...
        int ip = neighbor_zoneid;
        int tag = 0;
        std::printf( " SwapData( float * q ) 1£º this->zone_id =%d neighbor_zoneid = %d\n", this->zone_id, neighbor_zoneid );
        if ( neighbor_zoneid > this->zone_id )
        {
        ...
        {
            int n = -1;
            std::printf( " SwapData( float * q ) £º n =%d \n", n );
            MPI_Recv(&n, 1, MPI_INT, ip, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            std::printf( " SwapData( float * q ) recv £º n =%d \n", n );
            idata.recv_qs.resize( n, -1 );
            for ( int i = 0; i < idata.recv_qs.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 000: idata.recv_qs[%d]=%f \n", i, idata.recv_qs[ i ] );
            }
            MPI_Recv(idata.recv_qs.data(), n, MPI_FLOAT, ip, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for ( int i = 0; i < idata.recv_qs.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 111: idata.recv_qs[%d]=%f \n", i, idata.recv_qs[ i ] );
            }
        }
    }
    ...
    for ( int i = 0; i < this->neighbor_datas.size(); ++ i )
    {
        ...
        int ip = neighbor_zoneid;
        int tag = 0;
        std::printf( " SwapData( float * q ) 2£º this->zone_id =%d neighbor_zoneid = %d\n", this->zone_id, neighbor_zoneid );
        if ( neighbor_zoneid > this->zone_id )
        {
            int n = -1;
            std::printf( " SwapData( float * q ) 2 £º n =%d \n", n );
            MPI_Recv(&n, 1, MPI_INT, ip, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            std::printf( " SwapData( float * q ) 2 recv £º n =%d \n", n );
            idata.recv_qs.resize( n, -1 );
            for ( int i = 0; i < idata.recv_qs.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 2 000: idata.recv_qs[%d]=%f \n", i, idata.recv_qs[ i ] );
            }
            MPI_Recv(idata.recv_qs.data(), n, MPI_FLOAT, ip, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for ( int i = 0; i < idata.recv_qs.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 2 111: idata.recv_qs[%d]=%f \n", i, idata.recv_qs[ i ] );
            }
        }
        else
        {
...
void CfdSolve( int ni_global, float * xcoor_global, float simu_time, int zoneId, int ni, int nt, float cfl, float * xcoor, BoundarySolver * bcSolver )
{
    ...
    {
        int nproc = 4;
        std::vector<std::vector<float>> qvec(nproc);
        ...
        std::printf( "x_global.size=%zd\n", x_global.size() );
        std::printf( "q_global.size=%zd\n", q_global.size() );
