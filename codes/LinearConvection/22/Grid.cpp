...
void InterfaceSolver::SwapData( float * q )
{
    ...
    for ( int i = 0; i < this->neighbor_datas.size(); ++ i )
    {
        ...
        else
        {
            ...
            std::vector<float> tmp_ids(n,-1);
            for ( int i = 0; i < tmp_ids.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 000: tmp_ids[%d]=%f \n", i, tmp_ids[ i ] );
            }
            MPI_Recv(tmp_ids.data(), n, MPI_FLOAT, ip, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for ( int i = 0; i < tmp_ids.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 111: tmp_ids[%d]=%f \n", i, tmp_ids[ i ] );
            }
        }
    }
    ...
    for ( int i = 0; i < this->neighbor_datas.size(); ++ i )
    {
        ...
        if ( neighbor_zoneid > this->zone_id )
        {
            ...
            for ( int i = 0; i < tmp_ids.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 2 000: tmp_ids[%d]=%f \n", i, tmp_ids[ i ] );
            }
            MPI_Recv(tmp_ids.data(), n, MPI_INT, ip, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for ( int i = 0; i < tmp_ids.size(); ++ i )
            {
                std::printf( "SwapData( float * q ) 2 111: tmp_ids[%d]=%f \n", i, tmp_ids[ i ] );
            }
        }
        else
        {
