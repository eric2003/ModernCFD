...
void BoundarySolver::Init( int zoneId, int nZones, int ni )
{
    ...
    for ( int iZone = 0; iZone < nZones; ++ iZone )
    {
        ...
        if ( iZone == zoneId )
        {
            local_ptlist.insert( i0 );
            local_ptlist.insert( i1 );
        }
    }

    std::printf("BoundarySolver::Init pt: zoneId = %d nZones = %d\n", zoneId, nZones );
    std::printf("local pt list : " );
    for ( std::set<int>::iterator it = local_ptlist.begin(); it != local_ptlist.end(); ++ it )
    {
        std::printf("%d ", *it );
    }
    std::printf("\n");
    std::printf("global pt list : " );
