...
void InterfaceSolver::Init()
{
    ...
    //std::printf( " myzoneId =%d neighbor_zones.size() = %zd --------------------\n", zone_id, neighbor_zones.size() );
    ...
    //for ( auto it = this->neighbor_zones.begin(); it != this->neighbor_zones.end(); ++ it )
    //{
    //    int neighbor_zone_id =  *it;
    //    std::vector<int> interfaces;
    //    GetNeighborInterfaceId( neighbor_zone_id, interfaces );
    //    IData iData;
    //    iData.zone_id = neighbor_zone_id;
    //    iData.interface_ids = interfaces;
    //    this->neighbor_datas.push_back( iData );
    //}
    ...
    //{
    //    //iface -> local_interface->global_interface
    //    //(0,1)->(1,11)->(21,31)
...
void InterfaceSolver::GetNeighborInterfaceId( int neighbor_zone_id, std::vector<int> &interfaces )
{
    std::printf( " InterfaceSolver::GetNeighborInterfaceId this->interlist.size() = %zd\n", this->interlist.size() );
    for ( int i = 0; i < this->interlist.size(); ++ i )
    {
        IFaceBasic *f = this->interlist[ i ];
        std::printf( " myzone =%d neighbor_zone_id = %d global_face_id = %d face_id1 = %d face_id2 = %d zone1 =%d zone2 =%d\n",\
            this->zone_id, neighbor_zone_id, f->global_face_id, f->face_id1, f->face_id2, f->zone1, f->zone2 );
        int face_id = -1;
        if ( f->zone1 == neighbor_zone_id )
        {
            face_id = f->face_id1;
        }
        else if ( f->zone2 == neighbor_zone_id )
        {
            face_id = f->face_id2;
        }
        else
        ...
        std::printf( " face_id = %d\n ", face_id );
        interfaces.push_back( face_id );
    }
